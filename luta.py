...
def funcaoataque(tipoDataque, player, contra, personagem, nome_font):
    valor = tipoDataque-1
    pygame.mixer.Sound.play(haduken)
    if tipoDataque == 3:
        vida = player.atacar(valor)
    else:
        dano = player.atacar(valor)

    ataque_script = Candidatos['{}'.format(personagem)]['movimentos'][valor][2]
    ...
    return ANIMACAO_ATAQUE, text_ataque
...
def luta_screen(window, personagem):
    ...
    caixa_ataques = pygame.image.load(
        path.join(IMG_DIR, 'caixa_luta.png')).convert_alpha()
    caixa_ataques_small = pygame.transform.scale(
        caixa_ataques, (WIDTH, HEIGHT/4))
    ...
    while rodando:
            ...
            if estado == PODE_ATACAR:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_1:
                        estado, text_ataque = funcaoataque(
                            1, player, contra, personagem, nome_font)

                    # quando aperta 2
                    elif event.key == pygame.K_2:
                        estado, text_ataque = funcaoataque(
                            2, player, contra, personagem, nome_font)

                    # quando aperta 3
                    elif event.key == pygame.K_3:
                        estado, text_ataque = funcaoataque(
                            3, player, contra, personagem, nome_font)
                    # quando aperta 4
                    elif event.key == pygame.K_4:
                        estado, text_ataque = funcaoataque(
                            4, player, contra, personagem, nome_font)
                    else:
                        pass
        ...
        if estado == CATACANDO:
            if contra.hp > 0:
                ...
                ataque_contra_script = Candidatos['{}'.format(
                    contra.nome)]['movimentos'][tipo_cataque][3]
                text_ataque_contra = nome_font.render(
                    "{}".format(ataque_contra_script), True, (0, 0, 0))
       ...
       # dando print nos personagens e tela de fundo
        ...
        pygame.draw.polygon(window, BLACK, [(WIDTH/20, HEIGHT/40), (WIDTH/4.615,
                            HEIGHT/40), (WIDTH/4.615, HEIGHT/13.33333), (WIDTH/20, HEIGHT/13.33333)])
        pygame.draw.polygon(window, (0, 128, 0), [(WIDTH/20, HEIGHT/40), ((player.hp+WIDTH/20), HEIGHT/40), ((
            player.hp+WIDTH/20), HEIGHT/13.33333), (WIDTH/20, HEIGHT/13.33333)])
        # vida do contra (barra de vida em cima )
        pygame.draw.polygon(window, BLACK, [(WIDTH/1.27659, HEIGHT/40), (WIDTH/1.0526,
                            HEIGHT/40), (WIDTH/1.0526, HEIGHT/13.33333), (WIDTH/1.27659, HEIGHT/13.33333)])
        pygame.draw.polygon(window, (0, 128, 0), [(WIDTH/1.27659, HEIGHT/40), ((contra.hp+WIDTH/1.2765), HEIGHT/40), ((
            contra.hp+WIDTH/1.2765), HEIGHT/13.33333), (WIDTH/1.27659, HEIGHT/13.33333)])
        # caixa ataque blit
        if estado == PODE_ATACAR:
            window.blit(caixa_ataques_small, (0, (HEIGHT-HEIGHT/4)))
            for i in range(0, 4):
                ataque = Candidatos['{}'.format(
                    personagem)]['movimentos'][i][0]
                text_surface = nome_font.render(
                    "{}".format(ataque), True, (0, 0, 0))
            ...
            if player.hp <= 0 or contra.hp <= 0:
                ...
                if player.hp > 0 and contra.hp <= 0:
                    ...
                    pygame.mixer.music.load(
                        os.path.join(SND_DIR, 'derrota.mp3'))
                    pygame.mixer.music.play(loops=1)
                    return TELA, EMPATE
                else:
                    print('DERROTA')
